...
import com.ranecc.renderium.support.data.RealDataProvider;
 ...
 *
 * <p>负责根据相机到区块实体的距离动态分配 LOD 等级，
 * 支持视角依赖的细节调整和平滑过渡算法
 *
 * <h2>LOD 等级定义</h2>
 * <pre>
 * ┌───────┬──────────┬────────────┬────────────┬──────────────────┐
 * │ Level │ 名称     │ 距离范围   │ 多边形比率 │ 用途             │
 * ├───────┼──────────┼────────────┼────────────┼──────────────────┤
...
public final class LODCalculator {
     ...
     * 3. 如果所有阈值都未超过 → 返回最高 LOD 等级（最近最低细节）
     * </pre>
     *
     ...
     *
     * @param distance 到相机观察点的距离（区块数，必须 >= 0）
    ...
    /**
     * 从世界坐标平方距离计算超视距 LOD 等级（GPU 友好，无 sqrt）
     *
    ...
    /**
     * 从世界坐标平方距离计算 LOD 等级（公开 API，供外部调用者使用）
     *
     * <p>等价于 {@code calculateLODLevel(sqrt(distSqWorld) >> 4, fov)}，
     * 但通过平方距离比较避免昂贵的 sqrt 运算
     *
    ...
    /**
     * 使用 RealDataProvider 计算真实区块的 LOD 等级
     *
     * <h3>方法签名与参数说明：</h3>
     * <pre>
     * 参数：
     *   - provider: 数据提供者实例（必须已初始化，isReady() == true）
     *              类型: com.ranecc.renderium.data.RealDataProvider
    ...
    public int calculateLODLevelWithViewAdjustment(double distance, double fov,
        ...
        // 阈值设为 0.75，即 adjustment < 0.75 时提升一级
     ...
     *
     * <p>阈值数组必须严格递增（升序排列）。
     * 长度决定了 LOD 等级数量减一（n 个阈值 → n+1 个等级）
     *
    ...
    public void setMaxLevels(int maxLevels) {
        ...
        for (int i = 0; i < maxLevels - 1; i++) {
            // GPU优化：使用位移运算替代 Math.pow（2^i == 1 << i）
