...
public class LODPyramidBuilder {
    ...
    /**
     * LRU 缓存（使用 LinkedHashMap 的 accessOrder 特性）
     *
    ...
    public LODPyramidBuilder(int maxLODLevels, int cacheSize, boolean useGPUBuilding) {
        ...
        LOGGER.info(String.format(
            ...
            maxLODLevels, cacheSize, useGPUBuilding ? "✓" : "✗ (CPU)"
        ));
    }
     ...
     *     ↓ 2×2×2 平均下采样
     * Level 1 (LOD1): 每 8 个邻居取平均 (8×8×8 = 512 blocks)
     *     ↓ 2×2×2 平均下采样
    ...
    public void shutdown() {
        ...
        synchronized (cacheLock) {
            ...
            LOGGER.info(String.format(
                "LODPyramidBuilder 已关闭 (释放了 %d 个缓存条目)",
                cacheSizeBefore
            ));
        }
    }
