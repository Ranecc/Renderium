...
public class ShadowStyleNode extends AbstractPipelineNode {
    ...
    private static final String SHADER_PATH = "/shaders/shadow_style.spv";
    ...
    private void submitFullScreenDraw(RenderContext ctx, String pass, long inTex, long outTex, float[] uniforms) {
        ...
        try {
            ...
            MemorySegment cmdOut = PerFrameArena.allocate(ValueLayout.JAVA_LONG);
            ...
            if (fence == 0L) {
                MemorySegment fenceCI = PerFrameArena.allocateLongs(3);
                ...
                MemorySegment fenceOut = PerFrameArena.allocate(ValueLayout.JAVA_LONG);
            ...
            MemorySegment submitInfo = PerFrameArena.allocateLongs(7);
    ...
    private void ensurePipeline() {
        ...
        try {
            ...
            if (spirv == null || spirv.length == 0) {
                LOGGER.warning("[ShadowStyleNode] SPIR-V 加载失败，路径: " + SHADER_PATH);
                return;
            }
    ...
    private void ensureOutputImage(int width, int height) {
        ...
        try {
            ...
            MemorySegment imgOut = PerFrameArena.allocate(ValueLayout.JAVA_LONG);
            ...
            MemorySegment memOut = PerFrameArena.allocate(ValueLayout.JAVA_LONG);
            ...
            MemorySegment viewOut = PerFrameArena.allocate(ValueLayout.JAVA_LONG);
    ...
    private byte[] loadSPIRV() {
        try (InputStream is = getClass().getResourceAsStream(SHADER_PATH)) {
            if (is == null) {
                LOGGER.warning("[ShadowStyleNode] SPIR-V 资源不存在: " + SHADER_PATH);
                return null;
            }
            ...
            LOGGER.fine("[ShadowStyleNode] 加载 SPIR-V: " + SHADER_PATH + " (" + spirv.length + " bytes)");
            return spirv;
        } catch (Exception e) {
            LOGGER.warning("[ShadowStyleNode] SPIR-V 加载失败: " + e.getMessage());
            return null;
        }
    }
