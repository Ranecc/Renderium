...
public class RayTracingNode extends AbstractPipelineNode {
    ...
    protected boolean onInitialize(RenderContext context) {
        ...
        if (!rtAvailable) {
            LOGGER.warning("""
                    [RayTracingNode] Vulkan Ray Tracing 扩展不可用
                    节点将以回退模式运行（直通模式，不执行 RT 计算）
                    如需启用 RT，请确保：
                      1. 使用 Vulkan 后端（非 OpenGL）
                      2. GPU 支持 VK_KHR_ray_tracing_pipeline 扩展
                      3. 安装最新版显卡驱动
                    """);
        ...
        LOGGER.info("RayTracingNode 初始化完成: available=%b, enabled=%b, rays=%d, resolution=%dx%d"
    ...
    private boolean checkVulkanRTExtensions(RenderContext context) {
        ...
        LOGGER.warning("""
                [RayTracingNode] Vulkan RT 扩展检测尚未集成 VulkanStreamlineBridge
                所需扩展列表:
                  %s
                请在 Bridge 实现完成后启用此检测。
    ...
    private void buildAccelerationStructures(RenderContext context) {
        ...
        try {
            ...
            LOGGER.warning("""
                    [RayTracingNode] 加速结构构建尚未集成 VulkanStreamlineBridge
                    TLAS/SBT 句柄保持为 0L（未分配状态）
                    光线追踪功能将在 execute() 中回退为直通模式
                    """);
