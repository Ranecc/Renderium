...
public abstract class AbstractPipelineNode implements PipelineNode {
    ...
    /**
     * 返回此节点关联的 shader key，用于动态解析 SPIR-V
     * <p>
     * Shader key 格式如 "pipeline/lighting/shadow_style"，
     * 映射到 /shaders/pipeline/lighting/shadow_style.spv。
     * 无 shader 的节点返回 null（默认实现）。
     *
     * @return shader key 字符串，或 null 表示无关联 shader
     ...
     * 解析此节点的 SPIR-V 字节码
     * <p>
     * 通过 ShaderPathResolver 动态解析，支持 RGB 光影包覆盖。
     * 优先级：RGB 覆盖 > 预编译 SPV > 源码编译
     *
     * @return SPIR-V 字节码，如果 shaderKey() 为 null 或解析失败则返回 null
     */
    protected byte[] resolveSPIRV() {
        String key = shaderKey();
        if (key == null) return null;
        return com.ranecc.renderium.feature.shader.ShaderPathResolver.resolveSPIRV(key);
    }
