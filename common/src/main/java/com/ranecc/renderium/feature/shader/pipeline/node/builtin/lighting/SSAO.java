...
public class SSAO extends AbstractPipelineNode {
    ...
    protected String shaderKey() {
        return "pipeline/lighting/ssao";
    }

    /** Blur 子 shader key 常量 */
    private static final String KEY_BLUR = "pipeline/lighting/ssao_blur";
        ...
        synchronized (this) {
            ...
            try {
                ...
                if (r != null) {
                    ...
                    LOGGER.fine("SSAO Compute Pipeline 创建成功: " + shaderKey());
                }
            } catch (Exception e) {
    ...
    private void ensureBlurPipeline() {
        ...
        synchronized (this) {
            if (blurComputePipeline != 0L) return;
            try {
                byte[] spirv = com.ranecc.renderium.feature.shader.ShaderPathResolver.resolveSPIRV(KEY_BLUR);
                if (spirv == null || spirv.length == 0) {
                    LOGGER.warning("SPIR-V 着色器加载失败: " + KEY_BLUR);
                    return;
                }
                ComputePipelineHelper.Binding[] bindings = {
                    new ComputePipelineHelper.Binding(0, ComputePipelineHelper.VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER),
                ...
                if (r != null) {
                    ...
                    LOGGER.fine("SSAO Blur Compute Pipeline 创建成功: " + KEY_BLUR);
                }
            } catch (Exception e) {
