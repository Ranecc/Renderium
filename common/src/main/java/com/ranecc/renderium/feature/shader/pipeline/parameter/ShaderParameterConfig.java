...
public final class ShaderParameterConfig {
    ...
    private boolean uploadUniform(RenderContext context, String uniformName, ParameterKnob<?> knob) {
        switch (knob.getType()) {
            case FLOAT -> {
                float value = ((com.ranecc.renderium.feature.shader.pipeline.parameter.impl.FloatKnob) knob).getRawValue();
            ...
            case INT -> {
                int value = ((com.ranecc.renderium.feature.shader.pipeline.parameter.impl.IntKnob) knob).getRawValue();
                if (!com.ranecc.renderium.infrastructure.gpu.VulkanOperationGuard.isOk()) {
                    return true;
                }
                dirtyFlags.add(uniformName);
            ...
            case BOOL -> {
                boolean value = ((com.ranecc.renderium.feature.shader.pipeline.parameter.impl.BoolKnob) knob).getRawValue();
                if (!com.ranecc.renderium.infrastructure.gpu.VulkanOperationGuard.isOk()) {
                    return true;
                }
                dirtyFlags.add(uniformName);
            ...
            case ENUM -> {
                int index = ((com.ranecc.renderium.feature.shader.pipeline.parameter.impl.EnumKnob) knob).getCurrentIndex();
                if (!com.ranecc.renderium.infrastructure.gpu.VulkanOperationGuard.isOk()) {
                    return true;
                }
                dirtyFlags.add(uniformName);
