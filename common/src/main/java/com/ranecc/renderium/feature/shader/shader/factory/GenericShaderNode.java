...
package com.ranecc.renderium.feature.shader.shader.factory;
...
public class GenericShaderNode extends AbstractPipelineNode
    ...
    public long execute(RenderContext context, long... inputResources) {
        ...
        try {
            ...
            if (!gpuAvailable) {
                LOGGER.fine("[GenericShader] execute '%s' - GPU 资源管理器未初始化，降级为直通模式"
                        .formatted(getName()));
                return inputResources.length > 0 ? inputResources[0] : 0L;
            }
            ...
            if (descriptor.hasStorageImageOutput()) {
                ...
                if (outputRes.isValid()) {
                    ...
                    LOGGER.finest("[GenericShader] '%s' 创建输出纹理 0x%X (%dx%d)"
            ...
            LOGGER.fine("[GenericShader] execute '%s' (inputs=%d, uniforms=%d, workgroups=%dx%d)%s"
                    .formatted(getName(), inputResources.length, uniforms.size(),
            ...
            LOGGER.log(Level.WARNING, "[GenericShader] execute '%s' 异常，降级为直通"
                    .formatted(getName()), e);
            return inputResources.length > 0 ? inputResources[0] : 0L;
        }
    }
