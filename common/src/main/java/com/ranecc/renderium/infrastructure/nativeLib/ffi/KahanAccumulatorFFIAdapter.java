// Renderium Accelerator - Kahan/Neumaier高精度累加器 FFI适配器
// 数值精度接近f80 (80位扩展精度)
//
// 使用方法:
...
/**
 * Kahan/Neumaier高精度累加器 FFI适配器
 *
 * <p>通过补偿算法消除浮点累加中的精度损失，数值精度接近80位扩展精度。</p>
 * <p>GPU亲和度: 中 (批量可并行，单次CPU更优)</p>
 *
 * <h3>算法原理:</h3>
 * <pre>
 * 标准浮点累加问题: 1.0 + 1e100 + 1.0 - 1e100 = 0.0 (精度丢失!)
 * Kahan补偿:      1.0 + 1e100 + 1.0 - 1e100 = 2.0 ✓
 * </pre>
 *
...
public final class KahanAccumulatorFFIAdapter {
     ...
     * @return double[3] 数组:
     *         [0] sum - 当前累加和
     *         [1] compensation - 补偿值 (sum+compensation=真实结果)
     *         [2] elementCount - 已累加元素数
     */
