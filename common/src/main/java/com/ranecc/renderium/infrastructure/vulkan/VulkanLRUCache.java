 ...
 * <p>
 * 解决 {@code VulkanDevice.pipelineCache} �?{@code shaderCache} 的无界增长问题�? * 当缓存大小超过上限时，自动淘汰最久未使用的条目并销毁其 Vulkan 资源�? * </p>
 *
 * <h3>解决的问题：</h3>
 * <ul>
 ...
 * </pre>
 *
 * @param &lt;K&gt; 键类型（�?RenderPipeline / ShaderCompilationKey�? * @param &lt;V&gt; 值类型（需实现 Destroyable 接口或通过 evictionAction 销毁）
...
public class VulkanLRUCache<K, V> extends LinkedHashMap<K, V> {
    ...
    /**
     * 淘汰回调：当条目被淘汰时调用，用于销�?Vulkan 资源
     * 可以�?null（此时仅�?Map 中移除，不销毁资源）
     */
    ...
    /**
     * 淘汰动作函数式接�?     *
     * @param &lt;K&gt; 键类�?     * @param &lt;V&gt; 值类�?     */
    @FunctionalInterface
    public interface EvictionAction<K, V> {
        /**
         * 当缓存条目被淘汰时调�?         *
         * 【方法参数�?         * @param key   K - 被淘汰的�?         * @param value V - 被淘汰的�?         */
        void onEvict(K key, V value);
    }

    /**
     * 创建 LRU 缓存（带淘汰回调�?     *
     * 【方法参数�?     * @param maxSize        int - 最大缓存条目数（必�?> 0�?     * @param evictionAction EvictionAction - 淘汰时的回调（销�?Vulkan 资源），可以�?null
     *
     * 【异常�?     * @throws IllegalArgumentException 如果 maxSize <= 0
     */
    public VulkanLRUCache(int maxSize, EvictionAction<K, V> evictionAction) {
        // accessOrder=true 启用 LRU 顺序（基于访�?插入排序�?        super(16, 0.75f, true);
     ...
     * 【方法参数�?     * @param maxSize int - 最大缓存条目数
     */
     ...
     * 判断是否应淘汰最老的条目
     * <p>
     * LinkedHashMap �?put() 后自动调用此方法�?     * 返回 true 时，移除 eldest（最久未访问）的条目�?     * 并触�?evictionAction 回调以销毁关联的 Vulkan 资源�?     * </p>
     *
     * 【返回值�?     * @return boolean - true 表示应该淘汰最老条�?     */
    @Override
    protected boolean removeEldestEntry(Map.Entry<K, V> eldest) {
        if (size() > maxSize) {
            // 触发淘汰回调（销�?Vulkan 资源�?            if (evictionAction != null) {
                try {
    ...
    /**
     * 清空缓存并销毁所有资�?     * <p>
     * 对每个条目调�?evictionAction 后清�?Map�?     * </p>
     */
    ...
    /**
     * 创建线程安全的同步包�?     * <p>
     * 返回 Collections.synchronizedMap 包装后的实例�?     * 适合多线程环境下�?pipelineCache/shaderCache 使用�?     * </p>
     *
     * 【返回值�?     * @return Map&lt;K, V&gt; - 同步包装后的 LRU 缓存
     */
