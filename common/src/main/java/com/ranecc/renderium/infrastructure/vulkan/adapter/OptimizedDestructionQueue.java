 ...
 * 优化的销毁队�?(P1 优化)
 * <p>
 * 解决原版 {@code DestructionQueue.rotate()} 每次�?{@code new ReferenceArrayList()} 的性能问题�? * 改为复用已有容器，通过 clear() 清空后等待下次使用�? * </p>
 *
 * <h3>解决的问题：</h3>
 * <ul>
 *   <li>每次 rotate() 分配新容�?�?GC 压力</li>
 *   <li>双缓冲场景下每帧 2 次对象分�?/li>
 * </ul>
 *
 * <h3>优化原理�?/h3>
 * <pre>
 * 优化�? queues.set(index, new ReferenceArrayList&lt;&gt;())   // 每次分配
 * 优化�? queue.clear()                                  // 复用容器，零分配
 * </pre>
 *
 * @param &lt;T&gt; 资源类型（需实现 Destroyable 接口�? * @see com.renderium.vulkan.adapter.Destroyable
 * @since 5.2.0
 */
public final class OptimizedDestructionQueue<T> {
    ...
    /**
     * 创建优化的双缓冲销毁队�?     *
     * 【方法参数�?     * @param queueCount      int - 队列数量（通常�?2 �?3�?     * @param destroyCallback DestroyCallback - 资源销毁回�?     *
    ...
    public OptimizedDestructionQueue(int queueCount, DestroyCallback<T> destroyCallback) {
        ...
        if (destroyCallback == null) throw new IllegalArgumentException("destroyCallback 不能�?null");
    ...
    /**
     * 入队待销毁资�?     *
     * 【方法参数�?     * @param resource T - 待销毁的 Vulkan 资源
     */
    ...
    /**
     * 执行轮转并销毁资源（容器复用版本�?     * <p>
     * 与原版的区别：不�?new ReferenceArrayList()�?     * 而是获取现有容器、执行销毁、clear() 复用�?     * </p>
     *
     * 【返回值】boolean - true 表示有资源被销�?     */
    public boolean rotate() {
        // 切换到下一个队�?        ++currentIndex;
        ...
        // 批量销�?        int count = currentQueue.size();
        ...
        for (T resource : currentQueue) {
            try {
                ...
                LOGGER.warning("资源销毁异�? " + e.getMessage());
            }
        }

        destroyCallback.end();

        // 关键优化：清空容器而非创建新的（零 GC 开销�?        currentQueue.clear();
