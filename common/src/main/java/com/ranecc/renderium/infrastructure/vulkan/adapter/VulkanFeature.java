...
/**
 * VulkanFeature - Vulkan 可选功能特性标�?(26.2-snapshot-3 兼容)
 *
 ...
 *   <li>便于扩展新的 Vulkan 特性支�?/li>
 * </ul>
 *
 * <h3>使用示例:</h3>
 * <pre>{@code
 * // 检测设备支持的特�? * Set<String> extensions = getDeviceExtensions();
 * Set<VulkanFeature> supported = VulkanFeature.detectSupported(extensions);
 *
 * // 条件判断是否支持动态渲�? * if (supported.contains(VulkanFeature.DYNAMIC_RENDERING)) {
 *     // 使用动态渲染路�? * } else {
 *     // 回退到传�?RenderPass 路径
 * }
 * }</pre>
...
public enum VulkanFeature {
    ...
    /**
     * VK_KHR_dynamic_rendering - 动态渲染（�?RenderPass 对象�?     *
     * <p>Vulkan 1.2+ �?KHR 扩展，允许在不创�?RenderPass 对象的情况下进行渲染�?     * 显著简化渲染管线的创建和管理，减少 API 开销�?/p>
     *
     * <h4>优势:</h4>
     * <ul>
     *   <li>消除 RenderPass 对象创建开销</li>
     *   <li>更灵活的渲染流程控制</li>
     *   <li>与现代图�?API（DirectX 12、Metal）设计理念一�?/li>
     * </ul>
     */
     ...
     *
     * <p>允许直接将描述符数据推送到命令缓冲区，避免频繁更新描述符集合�?     * 对于每帧更新的数据（如变换矩阵、材质参数）特别有效�?/p>
     *
     * <h4>适用场景:</h4>
     * <ul>
     *   <li>频繁更新�?Uniform 数据</li>
     *   <li>减少描述符集合切换开销</li>
     *   <li>提高 CPU 端性能</li>
     * </ul>
     */
     ...
     *
     * <p>提供了更强大和灵活的同步机制，改进了屏障(barrier)和依赖关系的表达方式�?     * �?Vulkan 1.2 的核心特性之一�?/p>
     *
     * <h4>新功�?</h4>
     * <ul>
     *   <li>更精确的内存访问控制</li>
     ...
     *
     * <p>引入时间线信号量概念，支持细粒度的异步操作协调�?     * 允许 CPU �?GPU 更高效地并行工作�?/p>
     *
     * <h4>应用场景:</h4>
     * <ul>
     *   <li>异步计算队列同步</li>
    ...
    /**
     * VK_KHR_swapchain - 交换链支�?     *
     * <p>窗口系统集成的必要扩展，提供与显示器交换渲染结果的机制�?     * 几乎所有图形应用都需要此扩展�?/p>
     *
     * <h4>核心功能:</h4>
     * <ul>
     *   <li>Surface 创建与管�?/li>
     *   <li>SwapChain 图像获取与呈�?/li>
     *   <li>垂直同步与显示模式控�?/li>
     *   <li>全屏独占模式支持</li>
     * </ul>
     */
     ...
     *
     * <p>允许着色器使用 16 位半精度浮点数运算�?     * 可以显著提高 GPU 吞吐量和减少带宽占用�?/p>
     *
     * <h4>性能收益:</h4>
     * <ul>
     *   <li>在某�?GPU 上吞吐量翻�?/li>
    ...
    /**
     * VK_EXT_descriptor_indexing - 描述符索�?     *
     * <p>增强了描述符集合的功能，支持部分绑定、可变数量描述符等高级特性�?     * 对于实现纹理数组、延迟绑定等技术至关重要�?/p>
     *
     * <h4>关键特�?</h4>
     * <ul>
     *   <li>运行时描述符数组索引</li>
     ...
     *
     * <p>允许在着色器中直接使用缓冲区�?GPU 地址（指针）�?     * 为物理正确的光线追踪和高级数据结构提供基础支持�?/p>
     *
     * <h4>应用场景:</h4>
     * <ul>
     *   <li>光线追踪加速结构遍�?/li>
     ...
     * 获取对应�?Vulkan 扩展名称
     *
     * @return Vulkan 扩展字符串（�?"VK_KHR_swapchain"�?     */
    ...
    /**
     * 从扩展名集合检测支持的功能�?     *
     * <p>遍历所有已知的 Vulkan 特性，检查其扩展名是否存在于可用扩展集合中�?     * 返回所有匹配的特性枚举集合�?/p>
     *
     * <h4>算法复杂�?</h4>
     * <ul>
     *   <li>时间复杂�? O(n*m)，其�?n 是特性数量，m 是扩展集合查找时�?/li>
     *   <li>空间复杂�? O(k)，k 是支持的特性数�?/li>
     * </ul>
     *
     * @param availableExtensions 设备支持的扩展名集合（通常�?vkEnumerateDeviceExtensionProperties 获取�?     * @return 已支持的特性集合（EnumSet），不可变视�?     * @throws IllegalArgumentException 如果 availableExtensions �?null
     */
     ...
     * @throws IllegalArgumentException 如果 availableExtensions �?null
     */
     ...
     *
     * @param extensionName 要查找的扩展�?     * @return 对应�?VulkanFeature 枚举，如果没有找到则返回 null
     */
    ...
    /**
     * 获取所有必需的核心特性集�?     *
     * <p>返回大多数现�?Vulkan 应用所需要的基本特性集合�?     * 这些特性对于基本的图形渲染是必需的�?/p>
     *
     * @return 必需特性集�?     */
