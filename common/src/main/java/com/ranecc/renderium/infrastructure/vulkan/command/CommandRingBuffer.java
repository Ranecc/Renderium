 ...
 * <h3>核心设计理念</h3>
 * <ul>
 *   <li><b>�?GC 压力</b>：所有命令数据存储在堆外 MemorySegment 中，
 *       录制过程不产生任�?Java 对象</li>
 *   <li><b>紧凑二进制格�?/b>：每个命令打包为固定大小的记录，
 *       包含类型标识 + 参数载荷</li>
 *   <li><b>环形缓冲</b>：预分配固定容量，写入指针循环复用，
 *       无需动态扩�?/li>
 *   <li><b>无锁读取</b>：读取端通过快照机制安全遍历�? *       不需要读写锁同步</li>
 * </ul>
 *
 * <h3>命令记录布局（每个记�?64 字节�?/h3>
 * <pre>{@code
 * ┌──────────┬──────────┬──────────────────────────────────────�? * �?类型(1B) �?标志(1B) �?        载荷数据(62 bytes)           �? * └──────────┴──────────┴──────────────────────────────────────�? *
 * 载据根据类型有不同的解释�? * - DRAW:      pipelineHash(8) + mode(4) + count(4) + first(4) +
...
public final class CommandRingBuffer implements AutoCloseable {
     ...
     * 创建指定容量的环形缓冲区
     *
     * @param capacity 最大命令数量（必须�?2 的幂次方以优化取模运算）
     */
    public CommandRingBuffer(int capacity) {
        ...
        // 向上取整到最近的 2 的幂次方（优化取模为位与操作�?        this.capacity = roundUpToPowerOfTwo(capacity);
        ...
        // 初始化所有槽位为�?        this.buffer.fill((byte) 0);
        ...
        // 注册 Cleaner 安全网（防止用户忘记调用 close�?        String info = String.format("capacity=%d, totalBytes=%d", this.capacity, totalBytes);
     ...
     * 确保缓冲区未关闭
     *
     * @throws IllegalStateException 如果已关�?     */
     ...
     * <p>�?DrawCommand 数据直接写入堆外内存，不创建任何 Java 对象�?     *
     * @param pipelineHash RenderPipeline �?hashCode（用于管线查找）
     * @param drawMode     绘制模式 (GL_TRIANGLES=4, GL_LINES=1 �?
     * @param firstVertex  起始顶点索引
     * @param vertexCount  顶点数量
     * @throws IllegalStateException 如果不在录制状态或缓冲区已�?     */
    public void recordDraw(int pipelineHash, int drawMode, int firstVertex, int vertexCount) {
        ...
        // 写入类型和标�?        record.set(ValueLayout.JAVA_BYTE, OFFSET_TYPE, TYPE_DRAW);
        record.set(ValueLayout.JAVA_BYTE, OFFSET_FLAGS, (byte) 0x00); // 非索引模�?
        // 写入 DRAW 载荷
        ...
        record.set(ValueLayout.JAVA_INT, DRAW_INDEX_TYPE, 0);          // 无索�?        record.set(ValueLayout.JAVA_LONG, DRAW_INDEX_OFFSET, 0L);
    ...
    public void recordDrawIndexed(int pipelineHash, int drawMode,
        ...
        record.set(ValueLayout.JAVA_INT, DRAW_FIRST, 0);             // DrawElements �?firstVertex
     ...
     * @param srcHandle 源资�?handle
     * @param dstHandle 目标资源 handle
     * @param srcOffset 源偏移量（字节）
     * @param dstOffset 目标偏移量（字节�?     * @param size 拷贝大小（字节）
     */
     ...
     * 获取原始内存段（供高性能消费者直接访问）
     *
     * <p>返回底层 MemorySegment，高级用户可直接�?RECORD_SIZE 步进遍历�?     * 注意：此操作不安全，调用者需自行处理并发�?     *
     * @return 底层堆外内存�?     */
    ...
    /**
     * 获取指定位置的命令记�?     *
     * @param index 命令索引�? �?committedCount-1�?     * @return 该索引处的原始记录内存段（RECORD_SIZE 字节�?     */
    public MemorySegment getRecord(int index) {
        ensureNotClosed();
        if (index < 0 || index >= capacity) {
            throw new IndexOutOfBoundsException("index=" + index + ", capacity=" + capacity);
        }
        long offset = ((long) index & (capacity - 1)) * RECORD_SIZE; // 位与代替取模，加括号修正优先�?        return buffer.asSlice(offset, RECORD_SIZE);
    }

    /**
     * 获取指定位置的命令类�?     *
     * @param index 命令索引
     * @return 命令类型字节，TYPE_EMPTY 表示空槽�?     */
     ...
     * <p>将二进制数据解析到预分配数组中，避免每次调用产生 {@code new long[8]} 分配�?     * 适用于高频调用路径（如每帧遍历命令缓冲区）�?     *
     * <h3>输出数组布局</h3>
     * <pre>
     ...
     * </pre>
     *
     * @param record 命令记录内存�?     * @param output 预分配数组，长度必须 >= 8
     * @throws IllegalArgumentException 如果 output �?null 或长�?< 8
     */
     ...
     * 从记录中读取 Draw 命令参数
     *
     * <p>将二进制数据解析为参数数组，避免创建 DrawCommand 对象�?     * 返回数组: [pipelineHash, mode, count, first, indexType, indexOffset, instanceCount, baseInstance]
     *
     * @param record 命令记录内存�?     * @return 解析后的参数数组（长�?8�?     * @deprecated 使用 {@link #parseDrawParams(MemorySegment, long[])} 避免每次数组分配
     */
    @Deprecated
     ...
     * </pre>
     *
     * @param record 命令记录内存�?     * @param output 预分配数组，长度必须 >= 5
     * @throws IllegalArgumentException 如果 output �?null 或长�?< 5
     */
     ...
     * <p>将二进制数据解析到预分配 long 数组中，避免每次调用产生 {@code new Object[7]} 分配�?     * float 值通过 {@link Float#floatToRawIntBits(float)} 编码�?long 存储�?     * 使用 {@link #decodeFloat(long)} �?{@link #decodeInt(long)} 解码�?     *
     * <h3>输出数组布局</h3>
     * <pre>
     ...
     * </pre>
     *
     * @param record 命令记录内存�?     * @param output 预分配数组，长度必须 >= 7
     * @throws IllegalArgumentException 如果 output �?null 或长�?< 7
     */
     ...
     *
     * @deprecated 使用 {@link #parseClearParams(MemorySegment, long[])} 避免每次数组分配�?     *             float 值编码为 long，使�?{@link #decodeFloat(long)} 解码�?     */
    @Deprecated
    ...
    /**
     * �?long 解码�?int（用�?parseClearParams 零分配版本的 int 字段�?     *
     * @param encoded 编码后的 long �?     * @return 解码后的 int �?     */
    public static int decodeInt(long encoded) {
        return (int) encoded;
    }

    /**
     * �?long 解码�?float（用�?parseClearParams 零分配版本的 float 字段�?     *
     * @param encoded 通过 {@link Float#floatToRawIntBits(float)} 编码�?long �?     * @return 解码后的 float �?     */
    ...
    private static int roundUpToPowerOfTwo(int value) {
        if (value <= 0) return 1;
        if ((value & (value - 1)) == 0) return value; // 已经�?2 的幂
     ...
     * <p>此方法会�?     * <ol>
     *   <li>检查是否已关闭，防止重复释�?/li>
     *   <li>关闭 Arena，释放堆外内�?/li>
     *   <li>标记 closed 状�?/li>
     *   <li>记录资源释放日志</li>
     * </ol>
     *
     * <p>支持 try-with-resources 语法�?     * <pre>{@code
    ...
    public void close() {
        ...
        // 取消 Cleaner 注册（已手动关闭，不需要安全网�?        cleanable.clean();
        ...
        try {
            ...
            // Arena 可能已经被关闭（Cleaner 先执行），忽�?            LOGGER.fine("Arena was already cleaned up when close() was called");
        }
