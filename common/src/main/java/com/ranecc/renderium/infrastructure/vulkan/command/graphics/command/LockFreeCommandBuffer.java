// Renderium - 无锁命令缓冲区（Lock-Free SPSC Ring Buffer）
// 面向 1000+ FPS 的高性能设计，使用 CAS 原子操作替代锁
// 适用于单生产者-单消费者（SPSC）模式：录制线程/渲染线程
 ...
 *   <li><b>无锁写入</b>：使用 {@code volatile} + CAS 保证可见性，无锁竞争</li>
 *   <li><b>缓存行对齐</b>：读写指针分离到不同缓存行，避免 False Sharing</li>
 ...
 * <pre>{@code
 * ┌─────────────────────────────────────┐
 * │ Write Position (64B, cache-aligned) │ ← 生产者独占
 * ├─────────────────────────────────────┤
 * │ Read Position  (64B, cache-aligned) │ ← 消费者独占
 * ├─────────────────────────────────────┤
 * │ Ring Buffer Data (capacity × 64B)   │ ← 命令存储区
 * └─────────────────────────────────────┘
 * }</pre>
 *
 * <h3>命令记录格式（64 字节/条）</h3>
 * <pre>{@code
 * Offset  Size  Field
 * ─────────────────────────────
...
public final class LockFreeCommandBuffer implements AutoCloseable {
    ...
    public LockFreeCommandBuffer(int capacity) {
        ...
        // 分配堆外内存：读写指针 + 数据区
        long totalSize = CACHE_LINE_SIZE * 2 + (long) this.capacity * RECORD_SIZE;
        ...
        // 清零缓冲区
        this.buffer.asSlice(CACHE_LINE_SIZE * 2).fill((byte) 0);
    ...
    public void recordDraw(int pipelineHash, int mode, int count, int first) {
        ...
        // 写入命令数据（无序写入，最后写类型字段作为"提交"信号）
        record.set(ValueLayout.JAVA_INT, OFF_P1, pipelineHash);
        ...
        // 内存屏障：确保所有参数写入完成后，再写类型字段
        // volatile 写提供 StoreStore 屏障
    ...
    public void recordCompute(long pipelineHandle, int groupX, int groupY, int groupZ, long descriptorSet) {
        ...
        // volatile 写作为提交信号
        record.set(ValueLayout.JAVA_BYTE, OFF_TYPE, TYPE_COMPUTE);
        writePos = nextPos;
    }
    ...
    public int consumeAll(CommandHandler handler) {
        ...
        while (rPos != wPos) {
            ...
            // 清空类型字段（防止消费者追赶上生产者时误读）
            record.set(ValueLayout.JAVA_BYTE, OFF_TYPE, TYPE_EMPTY);
