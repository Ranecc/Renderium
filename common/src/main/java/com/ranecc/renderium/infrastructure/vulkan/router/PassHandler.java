// Renderium - Pass 处理器接�?// 用于定义自定�?Pass 处理器的标准接口

package com.renderium.router;

/**
 * Pass 处理器接�? *
 * <p>定义自定�?Pass 处理器的标准接口�? * 用于�?PassRouter 中注册和执行自定义渲染逻辑�? *
...
public interface PassHandler {
     ...
     * 处理 Pass 执行
     *
     * @param commandBuffer Vulkan 命令缓冲区句�?     */
    void handle(long commandBuffer);

    /**
     * Pass 执行前的回调（用�?ENHANCE 策略�?     *
     * @param commandBuffer Vulkan 命令缓冲区句�?     */
    default void onBeforePass(long commandBuffer) {
        // 默认空实�?    }

    /**
     * Pass 执行后的回调（用�?ENHANCE 策略�?     *
     * @param commandBuffer Vulkan 命令缓冲区句�?     */
