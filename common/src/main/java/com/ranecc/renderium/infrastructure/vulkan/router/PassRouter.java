...
// 用于在官方渲�?Pass 和自定义渲染 Pass 之间进行智能路由
// 来源文档: pass-routing-strategy.md
// 策略ID: ROUTE1 (Pass Routing Strategy #1)
// 预期收益: 灵活接管渲染流程，实�?DLSS/Reflex/FrameGen 等功�?
 ...
 * Pass 级智能路由器 🔀
 *
 * <p>在官方渲�?Pass 和自定义渲染 Pass 之间进行智能路由�? * 实现灵活接管渲染流程，为 DLSS/Reflex/FrameGen 等功能提供基础�? *
 * <h2>路由原理�?/h2>
 * <pre>
 * ┌─────────────────────────────────────────────────────────────�? * �?                   官方渲染流程                              �? * �? FrameGraph �?按依赖关系排�?�?执行每个 Pass                �? * �? �?TerrainPass, EntityPass, PostProcessPass, ...           �? * └─────────────────────────────────────────────────────────────�? *                              �? * ┌─────────────────────────────────────────────────────────────�? * �?                   Renderium 路由流程                        �? * �? PassRouter.intercept(passName)                            �? * �? �?查询路由表：是否应该接管�?Pass�?                       �? * �? �?是：执行自定�?PassHandler                               �? * �? �?否：放行给官�?Pass                                     �? * └─────────────────────────────────────────────────────────────�? * </pre>
 *
 * <h3>路由策略�?/h3>
 * <ul>
 *   <li><b>REPLACE</b>: 完全替代官方 Pass，执行自定义实现</li>
 *   <li><b>ENHANCE</b>: 在官�?Pass 前后执行额外操作</li>
 *   <li><b>PASSTHROUGH</b>: 完全放行给官�?Pass</li>
 * </ul>
 *
 * <h3>核心路由点：</h3>
 * <ul>
 *   <li><b>后处�?Pass</b>: 替换�?DLSS/Reflex 实现</li>
 *   <li><b>帧生�?Pass</b>: 插入 FrameGen 逻辑</li>
 *   <li><b>地形渲染 Pass</b>: 通常放行给官方实�?/li>
 *   <li><b>实体渲染 Pass</b>: 通常放行给官方实�?/li>
 * </ul>
 *
 * <h3>参考文档：</h3>
 * <ul>
 *   <li>pass-routing-strategy.md §1.0（Pass 路由策略�?/li>
 *   <li>dlss-integration-guide.md §2.1（DLSS �?Pass 路由�?/li>
...
public class PassRouter {
     ...
     */
    private PassRouter() {
        // 默认直�?Pass（官方擅长的 Pass�?        addPassthroughPass("terrain_solid");
        ...
        // 默认替换 Pass（我们擅长的 Pass�?        addReplacementPass("post_process", PostProcessHandler.getInstance());
        addReplacementPass("shadow_pass", ShadowPassHandler.getInstance());
        
        LOGGER.info("Pass 路由器已初始化，默认直�?" + passthroughPasses.size() + " �?Pass");
    }
     ...
     *
     * @param passName Pass 名称
     * @param commandBuffer Vulkan 命令缓冲区句�?     * @return true 如果已接管，false 如果放行
     */
