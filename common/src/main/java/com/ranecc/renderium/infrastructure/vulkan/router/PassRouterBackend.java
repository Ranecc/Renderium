...
/**
 * Pass 级智能路由器�? *
 * <p>设计来源：智能短�?md §第三类红利（极端情况下的"动态降级路�?�? *
 * <h2>核心策略</h2>
 * <p>把对官方管线的控制，�?非黑即白的短�?，改�?b>"Pass 级别的智能路�?</b>�? * VulkanBackend 在向 Queue 提交 CommandBuffer 时，不再是单一的提交，
 * 而是增加一层极薄的<b>路由判断�?/b>（纯位运算，没有性能损耗）�? *
 * <ul>
 *   <li><b>自己擅长的部�?/b>（GBuffer、基础光照、后处理超分）：
 *       走自己的 CommandBuffer，官方的对应 Pass 直接丢弃（位掩码抹零）�?/li>
 *   <li><b>官方超常发挥的部�?/b>（比如某个极度复杂的体积�?Compute Pass）：
 *       路由层识别到这个 Pass �?Tag，选择放行�? *       直接把官方准备好�?DescriptorSet �?CommandBuffer 原封不动地扔�?VkQueue�?/li>
 * </ul>
 *
 * <h2>路由决策模型</h2>
 * <p>每个渲染 Pass 有一个路由标签（RouteTag），决定其归属：
 * <pre>
 * ┌──────────────────────────────────────────────────────�? * �? RouteTag 枚举                                       �? * ├──────────────────────────────────────────────────────�? * �? RENDERIUM_OWNED  �?�?Renderium CommandBuffer       �? * �? OFFICIAL_PASSTHROUGH �?放行官方 CommandBuffer       �? * �? AUTO_DECIDE     �?根据 SPIR-V 缓存池自动决�?      �? * �? BLOCKED         �?完全丢弃（不执行�?               �? * └──────────────────────────────────────────────────────�? * </pre>
 *
 * <h2>性能保证</h2>
 * <ul>
 *   <li>路由判断使用位掩码（{@code long} 型），单次判�?�?1ns</li>
 *   <li>路由表使�?ConcurrentHashMap，线程安全，无锁读取</li>
 *   <li>不创建任何临时对象，�?GC 压力</li>
 * </ul>
 *
...
public final class PassRouter {
    ...
    public enum RouteTag {
        ...
        /** �?Renderium 自己�?CommandBuffer（默认） */
        RENDERIUM_OWNED(0b001),

        /** 放行官方�?CommandBuffer（直接提交到 VkQueue�?*/
         ...
         * 从位掩码值解�?RouteTag
         *
         * @param bits 位掩码�?         * @return 对应�?RouteTag
         */
    ...
    /**
     * Pass 路由�?     *
     * <p>Key: Pass 名称（如 "GBuffer"�?Lighting"�?VolumetricFog"�?     * Value: 路由标签的位掩码值（使用 int 而非 RouteTag 对象，避免对象分配）
     */
    ...
    /** 是否启用智能路由（false = 全部�?Renderium，即原来的短路模式） */
    ...
    /**
     * 初始化默认路由规�?     *
     * <p>根据当前运行模式和引擎能力，设置�?Pass 的默认路由�?     * 这些默认值可以在运行时通过 {@link #setRoute} 动态修改�?     */
    private void initializeDefaultRoutes() {
        ...
        // ===== 官方可能超常发挥的部分（自动决策�?=====
        // 如果官方有更好的实现（SPIR-V 缓存池里有），就放行；否则走自己�?        setRoute("VolumetricFog", RouteTag.AUTO_DECIDE);
     ...
     *
     * <p>这是路由器的核心方法，在渲染循环中每帧对每个 Pass 调用�?     * 必须保证极低的延迟（目标 �?1ns）�?     *
     * <h3>决策流程</h3>
     * <ol>
     *   <li>查路由表获取 RouteTag</li>
     *   <li>如果�?AUTO_DECIDE，查�?SPIR-V 缓存�?/li>
     *   <li>返回最终决�?/li>
     * </ol>
     *
     * @param passName Pass 名称
    ...
    public RouteTag resolveRoute(String passName) {
        ...
        // AUTO_DECIDE：查�?SPIR-V 缓存�?        if (tag == RouteTag.AUTO_DECIDE) {
     ...
     *
     * <p>逻辑�?     * <ul>
     *   <li>如果 SPIR-V 缓存池里有官方的�?Pass �?放行官方</li>
     *   <li>否则 �?�?Renderium 自己的实�?/li>
     * </ul>
     *
     * @param passName Pass 名称
     * @return 解析后的路由决策
    ...
    private RouteTag resolveAutoDecision(String passName) {
        ...
        if (spirvInterceptor.hasModule(passName)) {
            // 官方有更好的实现，放�?            LOGGER.fine("AUTO_DECIDE �?OFFICIAL_PASSTHROUGH: " + passName +
                    " (官方 SPIR-V 缓存命中)");
            return RouteTag.OFFICIAL_PASSTHROUGH;
        }

        // 官方没有，走自己�?        return RouteTag.RENDERIUM_OWNED;
    }

    /**
     * 快速判断指�?Pass 是否应该�?Renderium 管线
     *
     * <p>这是最高频调用的方法，用于渲染循环中的快速分支判断�?     * 使用位运算避免对象创建�?     *
     * @param passName Pass 名称
     * @return true 如果应该�?Renderium 自己�?CommandBuffer
     */
     ...
     *
     * @param passName Pass 名称
     * @return true 如果应该放行官方�?CommandBuffer
     */
    ...
    /**
     * 更新全局路由位掩�?     *
     * <p>用于不需要查表的快速判断场景�?     * 每个 Pass 的名称哈希值的�?6 位作为位索引�?     *
     * @param passName Pass 名称
     * @param tag 路由标签
     */
    private void updateGlobalMask(String passName, RouteTag tag) {
        ...
        if (tag == RouteTag.OFFICIAL_PASSTHROUGH) {
            mask |= (1L << bitIndex);  // 设置�?= 官方放行
        } else {
            mask &= ~(1L << bitIndex); // 清除�?= �?Renderium
        }
     ...
     * 启用/禁用智能路由
     *
     * <p>禁用后所�?Pass �?Renderium 自己的管线（回到原来的短路模式）�?     * 用于调试或在发现兼容性问题时快速回退�?     *
     * @param enabled true 启用智能路由，false 全部�?Renderium
     */
    public void setSmartRoutingEnabled(boolean enabled) {
        this.smartRoutingEnabled = enabled;
        LOGGER.info("智能路由 " + (enabled ? "已启�? : "已禁用（全部�?Renderium 管线�?));
    }
    ...
    public void resetToDefaults() {
        ...
        globalRouteMask = 0L;
        initializeDefaultRoutes();
        LOGGER.info("路由规则已重置为默认�?);
    }

    /**
     * 获取当前路由表的快照（用�?UI 显示�?     *
     * @return Pass 名称到路由标签的不可变映�?     */
