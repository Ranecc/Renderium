package com.ranecc.renderium.infrastructure.vulkan.vulkan;

import java.util.logging.Logger;

/**
 * Vulkan Submission 对象池 (P1 优化)
 * <p>
 * 解决 {@code VulkanQueue.close()} 中每次都分配 VkSubmitInfo2.Buffer、
 * SemaphoreSubmitInfo.Buffer 等栈内存的问题。
 * 通过对象池复用 Submission 对象，减少 GC 压力。
 * </p>
 *
 * <h3>解决的问题：</h3>
 * <ul>
 *   <li>每帧多次 MemoryStack 栈分配（beginSubmit/close）</li>
 *   <li>每个 SemaphoreOp / SubmitStage 都是新对象</li>
 *   <li>@1000+ FPS 下每秒数千次短生命周期对象分配</li>
 * </ul>
 *
 * @since 5.2.0
 */
public final class SubmissionPool {
     ...
     *
     * 【返回值】
     * @return PooledSubmission - 可用的 Submission 实例（已重置状态）
     */
     ...
     * <p>
     * 重置对象状态后放回池中供下次复用。
     * 如果池满则丢弃（不扩容，避免内存无限增长）。
     * </p>
     *
     * 【方法参数】
     * @param submission PooledSubmission - 要归还的 Submission
     */
