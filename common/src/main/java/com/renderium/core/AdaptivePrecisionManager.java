...
//
// 评分函数（三因子加权模型）：
//   score = 0.5 × ρ(空间复杂性) + 0.3 × r(ROI重要性) + 0.2 × (1-P)(资源余量)
//
// 资源感知阈值（动态调整）：
//   T_high(P) = min(T_high^0 × (1 + β×P), 0.95)
//   其中 P = 资源压力 [0,1], β = 压力敏感系数
//
// 集成关系：
//   - DynamicPrecisionManager: 提供全局精度基线和操作类别映射
//   - LyapunovQualityChecker: 用于质量反馈闭环（可选）
//   - PhaseTransitionDetector: 相变时重置历史数据
//
// 性能目标：
//   - 分析延迟 < 0.5ms/frame (1080p)
//   - GPU利用率提升 >40%
//   - PSNR损失 < 1dB (vs 全FP32基线)
//
 ...
 * @since 3.0.0
 * @see DynamicPrecisionManager
 * @see DynamicPrecisionManager.PrecisionLevel
 */
public final class AdaptivePrecisionManager {
     ...
     * <pre>{@code
     * // 方法1: 基于显存占用率
     * float memoryPressure = currentGPUMemoryUsage / totalGPUMemory;
     *
     * // 方法2: 基于帧时间超支比例
     * float timePressure = Math.max(0, (frameTimeMs - targetFrameTimeMs) / targetFrameTimeMs);
     *
     * // 方法3: 综合压力（推荐）
     * float combinedPressure = 0.6f * memoryPressure + 0.4f * timePressure;
     * adaptiveMgr.updateResourcePressure(combinedPressure);
     * }</pre>
     *
     ...
     * <h3>数学公式</h3>
     * <pre>{@code
     * r(x,y) = exp(-((x-cx)² + (y-cy)²) / (2σ²))
     *
     * where:
     *   (cx, cy) = 图像中心
   *   σ      = 标准差（控制衰减速度，默认为短边的1/3）
     * }</pre>
     *
     * @param xStart Tile左上角X
     * @param yStart Tile左上角Y
     ...
     * }</pre>
     *
     * 压力增大时，所有阈值同步提高，导致更多Tile被分配到更低精度，
     * 从而减少整体计算负载。
     *
