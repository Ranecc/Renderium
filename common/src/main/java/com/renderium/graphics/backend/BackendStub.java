// Renderium - 渲染后端存根接口
// 提供类型安全的占位实现，待 Blaze3D 迁移完成后替换为实际类型
//
// 此类解决了 OfficialVulkanHijacker 中 hijackedBackend 字段使用 Object 类型导致的类型安全问题
 ...
 * 渲染后端存根接口。
 * <p>
 * 这是一个类型安全的占位实现，用于替代 OfficialVulkanHijacker 中的 Object 类型存根字段。
 * 当迁移至 Blaze3D 后，可以无缝替换为 GpuDevice 或等效类型。
 *
 * <h3>设计目的：</h3>
 * <ul>
...
public interface BackendStub {
    ...
    final class BackendStubImpl implements BackendStub {
        ...
        private BackendStubImpl() {
            ...
            LOGGER.info("BackendStub 已创建（存根模式）- 待 Blaze3D 迁移后替换为 GpuDevice");
        }
        ...
        public Optional<Boolean> submitCommand(Object command) {
            ...
            try {
                ...
                if (command instanceof CommandBuffer cmdBuffer) {
                    // 验证命令缓冲区状态
                    if (cmdBuffer.getState() != CommandBuffer.State.EXECUTABLE) {
                        LOGGER.warning(String.format(
                            "BackendStub: CommandBuffer#%d 状态无效 (%s)，无法提交",
                    ...
                    if (commandCount == 0) {
                        LOGGER.fine("BackendStub: 提交空的 CommandBuffer，跳过执行");
                    ...
                    LOGGER.fine(String.format(
                        "BackendStub: 模拟提交 CommandBuffer#%d - %d 个命令 (%d 绘制, %d 计算)",
                        cmdBuffer.getBufferId(),
                        commandCount,
                ...
                // 计算提交耗时（用于性能监控）
                long submitDurationNanos = System.nanoTime() - submitStartTime;
