 ...
 * @since 3.0.0
 * @see RayTracingModule#buildTopLevelAS(InstanceData[], int)
 */
public final class InstanceData {
     ...
     * 多个不同的 InstanceData 可以引用同一个 blasHandle (实例化渲染)。
     *
     * <p>有效值: 通过 {@link RayTracingModule#buildBottomLevelAS} 创建的 BLAS 句柄</p>
     ...
     */
    public InstanceData(Matrix4f transform, long blasHandle) {
        if (transform == null) {
     ...
     * @param blasHandle BLAS 句柄
     * @return 配置好的 InstanceData
     */
    public static InstanceData createStatic(float x, float y, float z, long blasHandle) {
     ...
     * @return 配置好的 InstanceData
     */
    public static InstanceData createDynamic(Matrix4f transform, long blasHandle,
                                              int customIndex) {
        return new InstanceData(transform, blasHandle, (byte) 0xFF, 0, customIndex);
    }
    ...
    public String toString() {
        return String.format(
                "InstanceData{blas=0x%X, mask=0x%02X, hitGroup=%d, customIdx=%d, id=%d}",
