...
/**
 * NeoForge 平台的模式通知发送器实现。
 *
 * <p>实现 {@link RenderiumDualModeManager.ModeNotificationSender} 接口，
 * 使用 NeoForge API 向玩家发送模式切换通知。
 */
public final class NeoForgeNotificationSender implements RenderiumDualModeManager.ModeNotificationSender {
    ...
    public void sendNotification(RenderiumMode mode, Optional<String> sodiumVersion,
        ...
        switch (mode) {
            ...
            case FALLBACK -> {
                message.append("§6⚠ 降级模式§f - 最小化功能保证稳定性\n");
                message.append("§e   原因: 未检测到 Sodium 或存在兼容性问题\n");
            }
        }
